...
export interface CloudSpec {
  ...
  /**
   * Env var carrying a tenant-hosted grid's cloud-server URL (e.g. Digital.ai's
   * `DIGITALAI_CLOUD_SERVER`). Grids with a fixed hub (BrowserStack, LambdaTest)
   * omit this. When set, the inspector's cloud-connect payload's `cloudServer`
   * field is required and written to this env var before the session opens.
   */
